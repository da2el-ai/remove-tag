...
class WindowControl(QDialog):
  ...
  def startRemoveTag(self):
    self.removePath = self.ui.inputRemovePath.text()
    self.captionPath = self.ui.inputCaptionPath.text()
    self.isDeep = self.ui.checkDeep.checkState()
    self.backupExt = self.ui.inputBackupExt.text()
